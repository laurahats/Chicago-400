...
dfx2$CRIMESADD <- skipNApaste(dfx2$CRIMES1,dfx2$CRIMES2,dfx2$CRIMES3,dfx2$CRIMES4,dfx2$CRIMES5,dfx2$CRIMES6,dfx2$CRIMES7,
...
SOR1_1 <- sqldf("select *
                      from SOR1 
                      group by LNAME,FNAME,DOB, STREET, CITY, STATE
                      order by LNAME,FNAME,DOB, STREET, CITY, STATE")
...
fullSOR <- full_join(SOR1_1, SOR2_1, by = c("LNAME", "FNAME", "DOB", "STREET", "CITY", "STATE")) 

fullSOR2 <- fullSOR %>%
  mutate(
    ZIPCODE = case_when(!is.na(ZIPCODE.x) ~ ZIPCODE.x,
              TRUE ~ ZIPCODE.y),
    COUNTY = case_when(!is.na(COUNTY.x) ~ COUNTY.x,
              TRUE ~ COUNTY.y)
  ) %>%
  select(LNAME,FNAME,DOB,STREET,CITY,STATE,ZIPCODE,COUNTY)


write.csv(fullSOR2[1:40,], "~/SWB Projects/Chicago 400 Alliance/KnownAddresses_example.csv")

write.csv(fullSOR2, "~/SWB Projects/Chicago 400 Alliance/KnownAddresses.csv")
